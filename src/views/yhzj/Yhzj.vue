<template>
	<div id="body">
	  <table cellspacing="0">
	  				...
	  				<tr>
	  					<td width="46%">用户名</td>
	  					<td><input name="yhm" id="yhm" v-model="yhm"/><span>*</span></td>
							<td><div>{{tip}}</div></td>
	  				</tr>
	  				<tr>
	  					<td>密码</td>
	  					<td><input name="pwd" id="pwd" v-model="pwd"/><span>*</span></td>
	  				</tr>
	  				<tr>
	  					<td>确认密码</td>
	  					<td><input id="pwd2" v-model="pwd2"/><span>*</span></td>
	  				</tr>
	  				<tr>
...
<script>
	import{getAdd} from 'network/add.js'
	export default{
		name: 'Yhzj',
		data(){
			return{
			...
			save(){
				if(this.yhm){
					if(this.pwd&&this.pwd2){
						if(this.pwd==this.pwd2){
							getAdd(this.yhm,this.pwd).then(res=>{
								if(res==true){
									this.tip='保存成功'
								}else{
									this.tip='保存失败'
